...
$(document).ready(function () {
    ...
    $("#txtDocDate").on('input', debounce(function () {
        if ($.fn.DataTable.isDataTable("#tblPurchaseOrder")) {
            indentTable.ajax.reload();
        }
    }, 500));
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function bindItemMasterGrid() {
        ...
        var requestData = {
            ...
            orderColumn: null,
            orderDir: "asc",
            screenId: 'GRNEntry_103',//Need to change the screen id as per your data
            ...
            modelData: JSON.stringify({
                ...
                DllSupplier: $("#ddlSupplier").val() == 'All' ? '0' : $("#ddlSupplier").val(),
                DllLocation: $("#ddlLoation").val() == 'All' ? '0' : $("#ddlLoation").val()
            })
        };
        //End
        ...
        CommonAjaxMethod(virtualPath + 'Generic/GetPagingRecordsAgGridAsync', requestData, 'GET', function (response) {
            ...
            gridOptions = bindAgGrid("#myGrid", 'GRNReport', columnMeta, tableData);
            $("#customLoader").hide();
        });
    }
    ...
    indentTable = $('#tblPurchaseOrder').DataTable({
        ...
        "ajax": function (data, callback, settings) {
            var requestData = {
                ...
                orderColumn: data.order.length > 0 ? "DOCDdate" : null, //data.columns[data.order[0].column].data : null,
            ...
            //{"data":  "Srno"},
            {
                ...
                "render": function (data, type, row) {
                    if (type === "display" || type === "filter") {
                        return '<span><a href="ViewGrn?auth=' + AuthToken + '&id=' + row.ID + '&IsGRNView=1" data-toggle="tooltip" class="d-flex justify-content-between" title="" data-original-title="' + row.DocNo + '"><strong>' + row.DocNo + ' </strong> <span><img src="../../assets/images/icons/help/edit.png" class="icon-xsm"></span></a></span>';
                    }
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    ...
                    return '<div class="dropdown">' +
                        ...
                        '<div class="dropdown-menu actionmenu" aria-labelledby="dropdownMenuButton"> <a class="dropdown-item" href="ViewGrn?auth=' + AuthToken +'&id=' + row.ID + '">View Details</a>' +
                        isInactiveAction + 
                        ' <a  class="dropdown-item" href="../purchaseorder/index.html" aclass="dropdown-item" href="#">Print</a>' +
...
function onchangeLoation() {
    BindData();
}
function RedirectToPurchaseOrder(ctr) {

    var url = "../../MaterialManagement/Material/CreateGrn?auth=" + AuthToken +"&id=0&suppId=" + ctr.value;
...
function GetGrnItemId(GrnId, GrnDocNo) {
    ...
    CommonAjaxMethod(virtualPath + 'Generic/GetRecordsAsync', { modelData: jsonString, screenId: 'GRNEntry_101' }, 'GET', function (response) {
        ...
        for (var i = 0; i < grnItems.length; i++) {
            PrintARN_Label(grnItems[i].GRNItemId, GrnDocNo);
...
function printGRNOrder(GRN_Id, GRNDocNo) {
    $.ajax({
        url: '/Material/PrintGRNOrder?Auth=' + AuthToken, // API endpoint
        method: 'GET',
        data: { GRN_Id: GRN_Id, GRNDocNo: GRNDocNo },
        xhrFields: {
            responseType: 'blob' // Important to handle binary data like PDF
        },
        beforeSend: function (request) {
