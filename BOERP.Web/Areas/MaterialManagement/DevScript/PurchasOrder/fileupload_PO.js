...
function fileUploadWithPreview(dropArea) {
    ...
    const MAX_FILES = 10;
	const MIN_FILE_SIZE = 0 * 1024; // 0 KB
    ...
    function handleFiles(files) {
        files.forEach((file) => {
            ...
            if (!uploadedFiles.includes(file.name)) {
                ...
                fileDataCollection.push(file);
                var fileName = file.name;
                var fileType = file.type.split("/")[0];
                var type= file.type
                var fileUrl = "";
                var fFd = 0;
                previewFile(fileName, fileType, type, fileUrl, fFd);
            }
        });
    }

    // Preview the file based on its type
    function previewFile(fileName, fileType, type, fileUrl, fFd)
    {
...
function RemoveAttachFile(ctr)
{
        ...
        CommonAjaxMethod(virtualPath + 'ContentManagement/DeleteFileWithAuth', obj, 'POST', function (response)
        {
            ...
            if (fileDataCollection.length> 0) {
                fileDataCollection = fileDataCollection.filter(file => file.name !== fileName);
            }
            uploadedFiles = uploadedFiles.filter((name) => name !== fileName);
