...
let fileDataCollection4 = [];
let uploadedFiles4 = [];

function fileUploadWithPreview4(dropArea)  {

    const dragFile = document.getElementById("dragFile4"),
        button = document.getElementById("fileInputButton4"),
        input = document.getElementById("fileIndent"),
        documentImages4 = document.getElementById("documentImages4");
    ...
    function handleFiles(files) {
        files.forEach((file) => {
            ...
            // Validate file count
            if ($('.document-file4').length >= MAX_FILES) {
                showPopup(`You can upload a maximum of ${MAX_FILES} files.`);
                return;
            }

            // Avoid duplicates
            if (!uploadedFiles4.includes(file.name)) {
                uploadedFiles4.push(file.name);
                fileDataCollection4.push(file);
                ...
                previewFile4(fileName, fileType, type, fileUrl, fFd);
            }
        });
    }
    ...
    function previewFile4(fileName, fileType, type, fileUrl, fFd) {
        let previewElement;
        ...
        documentImages4.appendChild(newDocument);
...
document.querySelectorAll(".drag-area4").forEach(fileUploadWithPreview4);

//function RemoveAttachFile4(ctr) {
//    //debugger;
...
//            if (fileDataCollection4.length > 0) {
//                fileDataCollection4 = fileDataCollection4.filter(file => file.name !== fileName);
//            }
//            var newfileName = response.FileModel.NewFileName;
//            uploadedFiles4 = uploadedFiles4.filter((name) => name !== fileName);
//            fileModelListBOM = fileModelListBOM.filter(function (itemParent) { return (itemParent.NewFileName != newfileName); });
//            // Remove the closest parent container (.flexline or any other wrapper)
//            ctr.closest(".document-file4").remove();
//        });
...
function RemoveAttachFile4(ctr) {
    ...
    var fileDetails = ctr.id.split('||');
    var fileName = fileDetails[2]; // this is ActualFileName e.g., "13.XLS"

    // Remove from fileModelListBOM using ActualFileName
    fileModelListBOM = fileModelListBOM.filter(function (item) {
        return item.ActualFileName !== fileName;
    });

    // Remove the UI card / row
    ctr.closest(".document-file4").remove();
}



function DownloadIndentFile4(ctr) {
    var fileDetails = ctr.id.split('||');
