...
let fileDataCollection2 = [];
let uploadedFiles2 = [];

function fileUploadWithPreview2(dropArea) {
    const dragFile = document.getElementById("dragFile2"),
        button = document.getElementById("fileInputButton2"),
        input = document.getElementById("fileRefDocStep2"),
        documentImages2 = document.getElementById("documentImages2");
    ...
    function handleFiles(files) {
        files.forEach((file) => {
            ...
            // Validate file count
            if ($('.document-file2').length >= MAX_FILES) {
                showPopup(`You can upload a maximum of ${MAX_FILES} files.`);
                return;
            }

            // Avoid duplicates
            if (!uploadedFiles2.includes(file.name)) {
                uploadedFiles2.push(file.name);
                fileDataCollection2.push(file);
                ...
                previewFile2(fileName, fileType, type, fileUrl, fFd);
            }
        });
    }
    ...
    function previewFile2(fileName, fileType, type, fileUrl, fFd) {
        let previewElement;
        ...
        documentImages2.appendChild(newDocument);
...
document.querySelectorAll(".drag-area2").forEach(fileUploadWithPreview2);

function RemoveAttachFile2(ctr) {
    ...
    if (result) {
        ...
        var obj =
        {
            FileUrl: url, Id: id, Type: 1, ScreenId:"TechnicalDoc_101"
        }
        CommonAjaxMethod(virtualPath + 'ContentManagement/DeleteFileWithAuth', obj, 'POST', function (response) {
            // Remove the file from the uploadedFiles array
            if (fileDataCollection2.length > 0) {
                fileDataCollection2 = fileDataCollection2.filter(file => file.name !== fileName);
            }
            uploadedFiles2 = uploadedFiles2.filter((name) => name !== fileName);
            var newfileName = response.FileModel.NewFileName;
            fileModelList1 = fileModelList1.filter(function (itemParent) { return (itemParent.NewFileName!=newfileName); });
            // Remove the closest parent container (.flexline or any other wrapper)
            ctr.closest(".document-file2").remove();
        });
...
function DownloadIndentFile2(ctr) {
    var fileDetails = ctr.id.split('||');
