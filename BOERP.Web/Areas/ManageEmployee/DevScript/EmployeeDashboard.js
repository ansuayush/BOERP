$(document).ready(function () {
    ...
    LoadMasterDropdownTMS('dldBranch', obj3, 'All', false);
    ...
    LoadMasterDropdownTMS('dldYear', obj4, 'Select', false);

    $('#dldBranch').val(1).trigger('change');

    $('#dldBranch, #dldWeek, #dldYear').on('change', function () {
        getDashBoardDetails($('#dldEmployee').val(), $('#dldBranch').val(), $("#dldWeek").val(), $("#dldYear").val());
    });
...
function handleFinWeek(ctrl) {
    ...
    LoadMasterDropdownTMS('dldWeek', obj5, 'Select', false);
}
...
function setEmployee() {
    CommonAjaxMethodTMS(baseURL + 'ManageEmployee/GetEmployeeDashboard', null, 'GET', function (response) {
        console.log(response);

        EmployeeName = response.data.data.Table[0].EmployeeName;
        EmployeeID = response.data.data.Table[0].EmployeeID;
        $('#EmployeeName').html(EmployeeName)
        $('#dldEmployee').val(EmployeeID)


        setFinYear(0);

        getDashBoardDetails(EmployeeID, $('#dldBranch').val(), $("#dldWeek").val(), $("#dldYear").val());
