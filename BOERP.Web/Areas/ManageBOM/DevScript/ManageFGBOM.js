        ...
        if (localStorage.getItem('IsNew') == 'true') {
            ...
            CommonAjaxMethod(virtualPath + 'Generic/GetRecordsAsync', { modelData: jsonString, screenId: 'DocSeries_101' }, 'GET', function (response) {
                ...
                if (tblDocNo[0].DocLock == 0) {
                    $('#txtMIItemName').val(tblDocNo[0].DocNumber);
                    $('#txtMIItemName').prop('disabled', true);
                }

                if (response.data.data.Table1[0].IsItemCatIdMatchQC == 1) {
                    document.getElementById("hdnqcyes").value = 1;
                }
                else {
                    document.getElementById("hdnqcyes").value = 0;
                }
            ...
            $('#addnewfg').show();
            $('#addnewfg').html('&nbsp;Existing');

            $('#ddlItemName').prop('disabled', true);
            $('#ddlPackSize').prop('disabled', true);
        }

        $('#ddlBOMCategory').val('finishedgoods').trigger('change');
...
function LoadOtherCharges() {
	...
	<td>${i + 1}</td>
                <td>
                            ...
                            <td><input type="text" id="remark_${tblDocNo[i].ID}" oninput="HideErrorMessage(this)" class="form-control" value="" placeholder="Enter Text" /></td>             
           </tr>
...
function LoadOtherChargesEdit(tblDocNo, IsView) {
	...
	<td>${i + 1}</td>
                <td>
                            ...
                            <td><input type="text" ${IsView == `true` ? `disabled` : ``} id="remark_${tblDocNo[i].ID}" value="${tblDocNo[i].Comment}" oninput="HideErrorMessage(this)" class="form-control" value="" placeholder="Enter Text" /></td>             
           </tr>
...
function MaterialInformationAddRow(button) {
	...
	<td>${rowCount}</td>
                            ...
                            <td class="text-center">
                             ...
                             <td>
                                <select class="form-control applyselect MandatePP disableList" onchange="HideErrorMessage(this); setItemName(this)" id="subcate_${rowCount}">                                  
                                </select>
...
function MaterialInformationAddRowEdit(obj, IsView) {
	...
	<td>${rowCount}</td>
                            ...
                            <td class="text-center">
                                <input type="checkbox"   ${IsView == `true` ? `disabled` : ``} id="NewItem_${rowCount}" ${obj.CreateNew ? `checked` : ``}  onclick="ChangeItemDataPP(this)" name="FromClient" value="">
                                <label for="NewItem_${rowCount}" class="m-0 singlecheck pl-0"></label>
                            </td>
                             <td>
                                <select class="form-control applyselect MandatePP disableList"  onchange="HideErrorMessage(this); setItemName(this)" id="subcate_${rowCount}">                                  
                            ...
                            <div class="autocomplete-wrapper" style="position: relative;">
								...
								<input type="text" class="form-control searchlist MandatePP disableList"  ${obj.CreateNew ? `disabled` : ``}  value="${obj.Item_Code}" id="txtMIItemName_${rowCount}" oninput="Getdata(this); HideErrorMessage(this)" onclick="Getdata(this); HideErrorMessage(this)" placeholder="Type item..." autocomplete="off">
								<span id="sptxtMIItemName_${rowCount}" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!</span>
                                 <ul id="globalSuggestionBox_${rowCount}" class="suggestions"></ul>
    ...
    newRow.innerHTML = `
        ...
        <td>${rowNo}</td>
                            ...
                            <td class="text-center">
                             ...
                             <td>
                                <select class="form-control applyselect MandateSP disableList"  onchange="HideErrorMessage(this); setItemName(this)" id="subcate_${rowCount}">                                  
                                </select>
                                <span id="spsubcate_${rowCount}" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!</span>
                            </td>
                            ...
                            <div class="autocomplete-wrapper" style="position: relative;">
								...
								<input type="text" class="form-control searchlist MandateSP disableList" ${obj.CreateNew ? `disabled` : ``} value="${obj.Item_Code}" id="txtMIItemName_${rowCount}" oninput="Getdata(this); HideErrorMessage(this)" onclick="Getdata(this); HideErrorMessage(this)" placeholder="Type item..." autocomplete="off">
								<span id="sptxtMIItemName_${rowCount}" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!</span>
                                 <ul id="globalSuggestionBox_${rowCount}" class="suggestions"></ul>
                                 <input type="hidden" value="${obj.ItemId}" id="ddlItem_${rowCount}" />
				</div>
...
function MaterialInformationAddRowSP(button) {
	...
	<td>${rowNo}</td>
                            ...
                            <td><div class="autocomplete-wrapper" style="position: relative;">
								...
								<input type="text" class="form-control searchlist MandateSP disableList" id="txtMIItemName_${rowCount}" oninput="Getdata(this); HideErrorMessage(this)" onclick="Getdata(this); HideErrorMessage(this)" placeholder="Type item..." autocomplete="off">
								<span id="sptxtMIItemName_${rowCount}" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!</span>
                                 <ul id="globalSuggestionBox_${rowCount}" class="suggestions"></ul>
                                 <input type="hidden" id="ddlItem_${rowCount}" />
				</div>
...
function GetBOMDetById(id) {
    ...
    CommonAjaxMethod(virtualPath + 'Generic/GetRecordsAsync', { modelData: jsonString, screenId: 'ERP_Samp5_101' }, 'GET', function (response) {
        ...
        var pack = data.PackSize.split(' ');
        $("#ddlPackSize").val(pack[0]);
        if (pack[1]) {
            $('#unit_req_ul').val(pack[1]).trigger('change');
        }
        else {
            $('#unit_req_ul').val('ml').trigger('change');
        }

        $('#unit_req_ul').prop('disabled', true);
       ...
       // $('#ddlContactPerson option:selected').text()
        ...
        $('#txtItemSFG').val(data.SFG_Code)
        $('#SFGItemCode').html(data.SFG_ItemName)
        $('#ddlSFGSaleRate').val(data.SaleRate)
        $('#txtSFGQty').val(data.SFGQty)
        $("#txtRemarks").val(data.Remarks);
        $('#txtSAmpleId').val(data.SampleId);
        $('#totalRateSFG').html(((parseFloat(data.SFGQty * data.CONVERSION_FACTOR) || 0) * (parseFloat(data.SaleRate) || 0)).toFixed(2));
        ...
        $('#sfg_total').html(`₹ ` + ((parseFloat((data.SFGQty * data.CONVERSION_FACTOR)) || 0) * (parseFloat(data.SaleRate) || 0)).toFixed(2));
...
function SaveItemMaster(validationClass) {
    ...
    if (checkValidationOnSubmit(validationClass) == true) {
        ...
        var itemModel =
        {
            ...
            ITEM_NAME: $('#txtBrandName').val() + ' - ' + $("#ddlItemName").val() + ' - ' + $('#ddlPackSize').val() + ' ' + $('#unit_req_ul').val(),
            HSN_ID: $("#ddlHSN").val(),
            STATUS: 'Active',  // Active or Inactive
            ...
            LEAD_TIME: 0, 
            LEAD_TIME_TYPE: $("#ddlLoadTime").val(),
            PACK_SIZE: $("#ddlPackSize").val() + ' ' + $('#unit_req_ul').val(),
            TOLERANCE: 0,
            ...
            if (response.ValidationInput == 1) {
                ...
                if (itemIdFGNew > 0) {
                    ...
                    CommonAjaxMethod(virtualPath + 'Generic/GetRecordsAsync', { modelData: jsonString, screenId: 'ItemMaster_104' }, 'GET', function (response) {
                        ...
                        $('#savefg').text('Added');
                        $('#txtSFGQty').val($('#ddlPackSize').val());
                        $('#txtSFGQty').prop('disabled', true);
                        $('#newItemFG').hide();
                        CalculateRateSFG2();

                        BindItemToCustomer(item_list.ITEM_ID, $("#ddlCustomer").val(), $('#ddlContactPerson').val())
...
function BindItemToCustomer(item_id, customer_id, c_person) {

    var model475 = {
        id: 0,
        Type: 101,
        Item_Id: item_id,
        cust_comp: customer_id,
        c_person: c_person,
...
async function SaveBOMData() {
    ...
    if (checkValidationOnSubmit('Mandatory') == true && checkValidationOnSubmit('Mandate') == true && checkValidationOnSubmit('MandatePP') == true && checkValidationOnSubmit('MandateOther') == true)
    {
        ...
        if ($('#txtSAmpleId').val() !== '') {
            const row = ItemCodeList.filter(input => input.ERP_Sample_Det_UniqId == $('#txtSAmpleId').val());

            if (row.length <= 0) {
                FailToaster('Invalid SAmple ID!!!');
                return;
            }
        }

        const sfgItem = ItemCodeList.filter(input =>  input.ITEM_ID == $('#ddlItemSFG').val() && input.ITEM_CODE == $('#txtItemSFG').val());

        if (sfgItem.length <= 0) {
            FailToaster('Invalid SFG Item!!!');
            return;
        }

        const fgItem = ItemCodeList.filter(input => input.ITEM_ID == $('#ddlItem').val() && input.ITEM_CODE == $('#txtMIItemName').val());

        if (fgItem.length <= 0) {
            FailToaster('Invalid FG Item!!!');
            return;
        }

        const customer = CustomerCodeList.filter(input =>  input.PA_ID == $('#ddlCustomer').val() && input.ITEM_DISPLAY == $('#txtCustomerName').val());

        if (customer.length <= 0) {
            FailToaster('Invalid  Customer Name!!!');
            return;
        }
      ...
      //  let isGarbage = 0;
        ...
        if (localStorage.getItem('IsNew') == 'true' && $('#hdnBOMId').val() <= 0 && !$('#savefg').hasClass('btn-success')) {
       
            FailToaster('Please add the new finished good item!!!');
            return;

        }

        if ($('#totalRateSFG').html() <= 0) {

            FailToaster('The rate of semi-dinished goods must be greater than 0!!!');
            return;
        }
        ...
        const itemCode = ItemCodeList.find(input => input.ITEM_ID == $('#ddlItem').val())
            ...
            var FormulaInformation = {
                ...
                PackSize: $("#ddlPackSize").val() + ' ' + $('#unit_req_ul').val(),
                Quantity: $("#bomQty").val(),
                WastagePercent: 0,
                FormulatedBy: '',
                FMCode: '',
...
;        }
        ...
        if (model.Attatchements.length <= 0) {
            FailToaster('Please attatch atleast one file in Attachments before saving!!!');
            return;
        }
    ...
    CommonAjaxMethod(virtualPath + 'Generic/GetRecordsAsync', { modelData: jsonString, screenId: ScreenID }, 'GET', function (response) {
        ...
        {            
            if (from == 'txtMIItemName') {
                ...
                const pack = tableData[0].PACK_SIZE.split(" ");

                if (pack[0]) {
                    $('#ddlPackSize').val(pack[0]);

                    $('#txtSFGQty').val(pack[0]);
                }
                if (pack[1]) {
                    $('#unit_req_ul').val(pack[1]).trigger('change');
            ...
            else if (from == 'txtItemSFG')
            {
                ...
                $('#ddlUnitSFG').val(tableData[0].ALTERNATE_UNIT_TYPE > 0 ? tableData[0].ALTERNATE_UNIT_TYPE : 'Select').trigger('change');
                conv_fact_sfg = 0.001;
...
function GetdataMaster(inputElement, globalId, filtertype)
{
    ...
    else if (rowId == 'txtSAmpleId') {

        matchedItems = ItemCodeList.filter(item => {
            const display = (item.ERP_Sample_Det_UniqId || '').toLowerCase();
           
            const matchesSearch = display.includes(searchLower);
    ...
    if (matchedItems.length === 0) {
        ...
        matchedItems.forEach(item => {
            ...
            if (rowId != 'txtSAmpleId') {
                ...
                li.dataset.id = item.ITEM_ID;
                li.dataset.name = item.ERP_Sample_Det_UniqId;
                li.dataset.code = item.ERP_Sample_Det_UniqId;
            }
            ...
            li.addEventListener("click", function ()
            {
                ...
                if (rowId == 'txtMIItemName') {
                    ...
                    const cust = CustomerCodeList.find(input => input.PA_ID == item.c_id);

                    if (cust) {
                        $('#txtCustomerName').val(cust.ITEM_DISPLAY);
                        $('#ddlCustomer').val(cust.PA_ID);
                     
                        $('#txtBrandName').val(cust.BrandName)

                        var obj31 = {
                            parentId: cust.ERP_PARTY_TEMP_ID,
                            masterTableTypeId: 0,
                            isMasterTableType: false,
                            isManualTable: true,
                            manualTable: 98,
                        ...
                        LoadMasterDropdown('ddlContactPerson', obj31, '', false, item.Contact_P);
                    }
                   ...
                   GetItemMasterDetById(item.ITEM_ID, 'txtMIItemName','')
                    ...
                    $('#ddlUnitSFG').val(item.ALTERNATE_UNIT_TYPE > 0 ? item.ALTERNATE_UNIT_TYPE : 'Select').trigger('change');
                    conv_fact_sfg = 0.001
                    ...
                    $(`#ddlItemSFG`).val(item.ITEM_ID);
                    $(`#lblItemName`).text(item.ITEM_NAME);
                    $('#txtSAmpleId').val(item.ERP_Sample_Det_UniqId == null ? '' : item.ERP_Sample_Det_UniqId);
                    $('#ddlSFGSaleRate').val(item.SALE_RATE).trigger('change');
                    $('#ddlUnitSFG').val(item.ALTERNATE_UNIT_TYPE > 0 ? item.ALTERNATE_UNIT_TYPE : 'Select').trigger('change');
                    conv_fact_sfg = 0.001
                    $('#SFGItemCode').text(item.ITEM_NAME);
...
function CalculateRateSFG(ctrl) {
    ...
    if (ans == -1) {
        $('#totalRateSFG').html('0.00');
        $('#sfg_total').html('0.00');
        return;
    }

    $('#totalRateSFG').html((($('#txtSFGQty').val() * conv_fact_sfg) * $('#ddlSFGSaleRate').val()).toFixed(2));

    $('#sfg_total').html((($('#txtSFGQty').val() * conv_fact_sfg) * $('#ddlSFGSaleRate').val()).toFixed(2));
...
function GetItemDetails(ITEM_ID, rowId) {
    ...
    CommonAjaxMethod(virtualPath + 'Generic/GetRecordsAsync', { modelData: jsonString, screenId: 'ERP_Samp5_101' }, 'GET', function (response) {
        ...
        $('#sale_' + rowId).val(data.SALE_RATE / 1000).trigger('change');
        $('#purch_' + rowId).val(data.PURCH_RATE / 1000);
...
function CalculateRate2(rowId) {
    ...
    $('#totlCost_' + rowId).html((parseFloat($('#txtMIQty_' + rowId).val() || 0) * parseFloat($('#txtMISellingPrice_' + rowId).val() || 0)).toFixed(2));
