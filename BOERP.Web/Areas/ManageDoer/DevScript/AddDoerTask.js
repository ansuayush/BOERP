...
function BindQuestion() {
    for (let i = 0; i < quesArray.length; i++) {
        if (quesArray[i].Type == 'Numerical') {
            ...
            var element = '<div class="checkdgin">' +
                '<span class="badge badge-secondary badgeinfo">Priority ' + quesArray[i].Priority  + '</span>' +
                '<div class="d-flex questiondtal">' +
                ques +
                '</div>' +
                '<h6 class="questiontittle fw-n">' + quesArray[i].Question + '</h6> ' +
            ...
            if ($("#hdnStatus").val() == 2) {
                ...
                var text = '<textarea placeholder="Enter" class="font-contorl question-form-contorl h-100 Mandatory" id="num' + quesArray[i].Id + '" disabled >' + (quesArray[i].UserText == null ? "Enter" : quesArray[i].UserText) + '</textarea>';
            ...
            if ($("#hdnStatus").val() == 1) {
                for (let j = 0; j < optionText.length; j++) {
                    option = option +
                        ...
                        '<input name="option' + quesArray[i].Id + '" type="radio" id="Option' + optionText[j].ID + '" onchange="handleError(' + quesArray[i].Id + ')" class="radio" >' +
                        '<label for="Option' + optionText[j].ID + '" >' + optionText[j].OptionText + '</label>' +
                        '</div>' +
                        '</li>';
                ...
                for (let j = 0; j < optionText.length; j++) {
                    if (optionText[j].IsSelect == true) {
                        option = option +
                            '<li class="singleselect">' +
                            '<div class="radio rediodgn">' +
                            '<input name="option' + quesArray[i].Id + '" type="radio" id="Option' + optionText[j].ID + '"  checked class="radio" >' +
                        ...
                        option = option +
                            '<li class="singleselect">' +
                            '<div class="radio rediodgn">' +
                            '<input name="option' + quesArray[i].Id + '" type="radio" id="Option' + optionText[j].ID + '"  selected disabled class="radio" >' +
                            '<label for="Option' + optionText[j].ID + '" class="radio-label">' + optionText[j].OptionText + '</label>' +
                            '</div>' +
                            '</li>';
                    }
...
function SaveDoerTask() {
    ...
    var isBool = true;
    for (let i = 0; i < quesArray.length; i++) {
        if (quesArray[i].IsMandatory == true & (quesArray[i].Type == 'singleselect' || quesArray[i].Type == 'multiplechoice')) {
    ...
    if (checkValidationOnSubmit('Mandatory') == true) {
        ...
        CommonAjaxMethodTMS(baseURL + 'ManageDoer/SaveDoerModel', DoerModel, 'POST', function (response) {
            if (response.ValidationInput == 1) {
                sessionStorage.setItem('successShown', 'true');
                sessionStorage.setItem('message', '<span class="checkmark">✅</span> Task Submitted Successfully');
