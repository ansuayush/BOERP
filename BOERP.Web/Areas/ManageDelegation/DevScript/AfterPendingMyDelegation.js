...
function EditDelegationMaster(id) {
    ...
    if (isCompleted == 1) {
        CommonAjaxMethodTMS(baseURL + 'DelegationTransaction/GetMyDelegation', { id: id, Type: 4}, 'GET', function (response) {
        ...
        CommonAjaxMethodTMS(baseURL + 'DelegationTransaction/GetMyDelegation', { id: id, Type: 3}, 'GET', function (response) {
            console.log(response);
            var data = response.data.data.Table[0];
            var files = response.data.data.Table1;
            fillValues(data, files);
...
function SaveAuditTask() {
    ...
    if (!$("#audityes").is(":checked") && !$("#auditno").is(":checked")) {
        ...
        if ($("#auditRemark").val() == '' || $("#auditRemark").val() == null) {
            $("#auditRemark").val('No Remark');
        }
    ...
    if (checkValidationOnSubmit('Mandatory')) {
        ...
        CommonAjaxMethodTMS(baseURL + 'DelegationTransaction/SaveDelegationAuditTask', auditObj, 'POST', function (response) {
            if (response.ValidationInput == 1) {
                sessionStorage.setItem('successShown', 'true');
                sessionStorage.setItem('message', '<span class="checkmark">✅</span> Delegation Approved Successfully');
...
function changeRemark(test) {
    $('#spaudit').hide();
    if (test == "auditno") {
        $("#VerifyRemark").html('Remarks<sup class="red-clr">*</sup>');
    }
    else {
        $("#VerifyRemark").html("Remarks")
    }
}